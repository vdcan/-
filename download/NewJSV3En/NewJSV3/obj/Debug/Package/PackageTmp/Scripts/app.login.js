...
var doRetrieve = function () {
    ...
    obj.email = $("#email").val(); 
    if (obj.user_code == "" &&obj.email == "") { layer.msg(gDictionary["please_input_email"]); return; } 
  //  $(".login_msg").html('<span style="color:red; font-weight:bold"><img src="/Content/images/ajax-loader.gif" />'+gDictionary["logining"]+'</span>');
    com.ajax({
        url: '/Login/ForgetPWD', data: JSON.stringify(obj), showLoading: false, success: function (result) {
           // alert(JSON.stringify(result))
            if (result.s == 0) {
