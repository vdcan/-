...
$.extend($.fn.validatebox.defaults.rules, {
    ...
    intOrFloat: {// 验证整数或小数  
        ...
        message: 'Please input an int or a float'
    },
    currency: {// 验证货币  
        validator: function (value) {
            return /^\d+(\.\d+)?$/i.test(value);
...
$.fn.dialog_ext2 = function (opts, cancel_text) {
    var query = parent.$;
    var self = this;
    //query.parser.parse($(this));
    var defaults = {
    ...
    var btns = {
        ...
        buttons: [{
            ...
            text: cancel_text, iconCls: 'icon-cancel', handler: function () { win.dialog('destroy'); }
        }]
    }
    opts = query.extend(btns, opts);
    query.parser.parse(win);
