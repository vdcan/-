     ...
     $('#TPL_procedure_name').combobox({
       ...
       //     alert('');
           var options = $("#TPL_parameter_name").combobox('options')
            ...
            if (u != undefined) {
               var jsonParam = JSON.stringify({ proc: c });
        ...
        this.jq = $("#Base_Procedure_Parametersgrid").datagrid({
            ...
            columns: [[
 	                ...
 	                { field: 'id', title: '编号', width: 40, align: 'center', sortable: true },
                            { field: 'procedure_name', title: '存储过程名', width: 300, align: 'left', sortable: true },
                            { field: 'parameter_name', title: '参数名', width: 100, align: 'left', sortable: true },
                  //          { field: 'parameter_type', title: '参数类型', width: 80, align: 'center', sortable: true },
                 //           { field: 'max_length', title: '长度', width: 80, align: 'center', sortable: true },
                 //           { field: 'is_out', title: '输出标志', width: 80, align: 'center', sortable: true,formatter: function (value, row, index) {
                 //var h = '未定义';
             
                 //                   if(value=='0') h ='输入'; 
                 //                   if(value=='1') h ='输出';                                    return h;
                 //              } 
                 //                },
                                                { field: 'From_Where', title: '从哪得值', width: 80, align: 'center', sortable: true,formatter: function (value, row, index) {
                 var h = '未定义';
...
km.toolbar = {
    ...
    do_edit: function () {
        ...
        if (km.Base_Procedure_Parametersgrid.selectedRow == null) { layer.msg('请选择一条记录！'); return; }
        km.set_mode('edit');
    ...
    do_delete: function () {
        ...
        com.message('c', ' <b style="color:red">确定要删除存储过程参数【' + sRow.id + '】吗？ </b>', function (b) {
            if (b) {
                com.ajax({
