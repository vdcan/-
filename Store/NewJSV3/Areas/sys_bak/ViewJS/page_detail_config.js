...
km.xx_page_detail_configgrid = {
    ...
    init: function () {
        this.jq = $("#xx_page_detail_configgrid").datagrid({
            ...
            columns: [[{ field: 'ColumnName', title: '列名', width: 250, align: 'left', sortable: true },
                          ...
                          { field: 'ColumnCaption', title: '列标题', width: 280, align: 'left', sortable: true, editor: { type: 'textbox', options: {} } },
...
{
    field: 'isShow', title: '<input    type="checkbox"/>s', width: 60, align: 'center', sortable: false, formatter: function (value, row, index) {
        var str = '';
        if (value == 1)
    ...
    field: 'isInsert', title: '<input    type="checkbox"/>i', width: 60, align: 'center', sortable: false, formatter: function (value, row, index) {
        var str = '';
        if (value == 1)
            str = '<input disable=true checked type="checkbox"/>'
        else
        ...
        return str;
                            ...
                            {
                                field: 'isUpdate', title: '<input  onclick="ClickMe(1,this )"  type="checkbox"/>u', width: 60, align: 'center', sortable: false, formatter: function (value, row, index) {
                                    var str = '';
                                    if (value == 1)
                            ...
                            {
                                field: 'isWhere', title: '<input    type="checkbox"/>w', width: 60, align: 'center', sortable: false, formatter: function (value, row, index) {
                                    var str = '';
                                    if (value == 1)
                                        str = '<input disable=true checked type="checkbox"/>'
                                    else
                         ...
                         //   { field: 'width', title: '宽度', width: 50, align: 'center', sortable: true, editor: { type: 'textbox', options: {} } },
                          ...
                          //},

                           {
                             ...
                             // { field: 'isRequired', title: '必填', width: 50, align: 'center', sortable: true, editor: { type: 'textbox', options: {} } },





                           //   { field: 'ColumnType', title: '列类型', width: 100, align: 'center', sortable: true, editor: { type: 'textbox', options: {} } },


                              { field: 'ColumnLength', title: '列长度', width: 50, align: 'center', sortable: true, editor: { type: 'textbox', options: {} } },


                                {
                                    field: 'htmlType', title: 'html类型', width: 120, align: 'center', sortable: true, formatter: function (value, row) {
                                        return value;
                                    },
                                    editor: {
                                        type: 'combobox',
                              ...
                              { field: 'staticValue', title: '静态数据', width: 250, align: 'center', sortable: true, editor: { type: 'textbox', options: {} } },
            ...
            onLoadSuccess: function (data) {
               // console.log(data.footer[0].files_text);
                ...
                tmp = tmp.replaceAll("]", ',]')
                if (tmp.indexOf(",s,") < 0)
                    $('#xx_page_detail_configgrid').datagrid('hideColumn', 'isShow');
                if (tmp.indexOf(",u,") < 0)
                    $('#xx_page_detail_configgrid').datagrid('hideColumn', 'isUpdate');
                if (tmp.indexOf(",i,") < 0)
                    $('#xx_page_detail_configgrid').datagrid('hideColumn', 'isInsert');
                if (tmp.indexOf(",w,") < 0)
                    $('#xx_page_detail_configgrid').datagrid('hideColumn', 'isWhere');
