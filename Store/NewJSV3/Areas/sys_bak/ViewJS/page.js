        ...
        this.jq = $("#xx_pagegrid").datagrid({
            ...
            columns: [[
 	                ...
 	                { field: 'id', title: '编号', width: 40, align: 'center', sortable: true },
                         //   { field: 'template_id', title: '模板', width: 80, align: 'center', sortable: true },
                            { field: 'page_name', title: '页面名称', width: 180, align: 'left', sortable: true },
                            { field: 'description', title: '描述', width: 280, align: 'left', sortable: true },
                            {
                                field: 'parent_menu_code', title: '', width: 40, align: 'center', sortable: false, formatter: function (value, row, index) {

                                    return '<a href="javascript:GoToPage(' + row.id + ')" >程序</a>';
                                }
                            },{
                                field: 'check', title: '', width: 80, align: 'center', sortable: false, formatter: function (value, row, index) {

                                    return '<a href="javascript:edit_sub_page(' + row.id + ',0)" >子页面</a>';
                                }
                            },
                        //    { field: 'parent_menu_code', title: '父菜单', width: 80, align: 'center', sortable: true },
            ...
            onLoadSuccess: function (data) {
                ...
                for (var i = 0; i < detail.length; i++) {
                    ...
                    var tmps = "";
                    //if (detail[i].DepartmentName == "")
                    //    tmps = String.format('<div id="d{0}" style="float:left; color:red;">{1} ,  </div>',
                    //    detail[i].id, detail[i].check_items, detail[i].DepartmentName
                    //    );
                    //else
                    tmps = String.format('<div id="d{0}" class="sub_page"> <div class="sub_page_title">  {1} </div> <a href="javascript:EditSubPage({0})">编辑</a>&nbsp;|&nbsp;<a href="javascript:DeleteSubPage({0})">删除</a>&nbsp;|&nbsp;<a href="javascript:GenSubPage({0})">生成</a> &nbsp;|&nbsp<a href="javascript:GoToPage({0})" >程序</a>   </div>',
...
km.toolbar = {
    ...
    do_edit: function () {
        ...
        if (km.xx_pagegrid.selectedRow == null) { layer.msg('请选择一条记录！'); return; }
        km.set_mode('edit');
    ...
    do_delete: function () {
        ...
        com.message('c', ' <b style="color:red">确定要删除页面【' + sRow.id + '】吗？ </b>', function (b) {
            if (b) {
                com.ajax({
...
function InitMenuTree() {
    $('#TPL_parent_menu_code').combotree({
        url: km.model.urls["tree"], editable: false, valueField: 'MenuCode', textField: 'MenuName', loadFilter: function (data) {
            //   data: gMenuData, editable: false, loadFilter: function (data) {
            var d = utils.copyProperty(data.rows || data, ["id", "IconClass", "MenuName", "MenuCode"], ["id2", "iconCls", "text", "id"], true);
           // console.log(d);
            var resultData = utils.toTreeData(d, 'id2', 'ParentID', "children");
            return resultData;
        }
    });
}
    ...
    init: function () {
        this.jq = $("#xx_page_detailgrid").datagrid({
            ...
            columns: [[
                       ...
                       //      { field: 'page_id', title: '页面编号', width: 50, align: 'center', sortable: true, editor: { type: 'textbox', options: {} } },


                              {
                                  field: 'area', title: '区域', width: 50, align: 'center', sortable: true,
                                  editor: {
                                      type: 'combobox',
                                      options: {
                                ...
                                {
                                    field: 'table_name', title: '表名', width: 280, align: 'left', sortable: true, formatter: function (value, row) {
                                        return value;
                                    },
                                    editor: {
                                        type: 'combobox',
                                            ...
                                            onSelect: function (data) {
                                                ...
                                                for (var i2 = 0; i2 < table_list.rows.length ; i2++)
                                                    if (table_list.rows[i2].id + "" == value) {
                                                        ...
                                                        target2.textbox("setValue", table_list.rows[i2].TableDescription);
                                                        target3.textbox("setValue", table_list.rows[i2].TableDescription);
                                                        target4.textbox("setValue", table_list.rows[i2].TableDescription);
                                                    }
                                            }
                                  ...
                                  { field: 'class_name', title: '类名', width: 180, align: 'left', sortable: true, editor: { type: 'textbox', options: {} } },


                                {
                                    field: 'module_id', title: '模块名', width: 280, align: 'left', sortable: true, formatter: function (value, row) {
                          ...
                          { field: 'table_description', title: '注释', width: 380, align: 'left', sortable: true, hidden: true, editor: { type: 'textbox', options: {} } },
                          { field: 'comments', title: '注释', width: 280, align: 'left', sortable: true, editor: { type: 'textbox', options: {} } },
                          {
            ...
            onLoadSuccess: function () {
                $("a[name='opera']").linkbutton({ text: '设置', plain: true, iconCls: 'icon-add' });
            }
        });//end grid init
    },
    reload: function (queryParams) {
...
function removeit_xx_page_detail() {
    if (editIndexxx_page_detail == undefined) { layer.msg('请选择一条记录！'); return; }

    var sRow = km.xx_page_detailgrid.getSelectedRow();
    if (sRow == null) { layer.msg('请选择一条记录！'); return; }
    //if (km.model.loginer.IsSuperAdmin == 0 && sRow.AllowEdit == 0) { layer.msg('此参数不可删除！'); return; }
    var jsonParam = JSON.stringify(sRow);
    com.message('c', ' <b style="color:red">确定要删除页面详细【' + sRow.id + '】吗？ </b>', function (b) {
        if (b) {
            com.ajax({
...
function DeleteSubPage(id) {
    ...
    var jsonParam = {id:id};
    com.message('c', ' <b style="color:red">确定要删除该子页面吗？ </b>', function (b) {
        if (b) {
            com.ajax({
                url: km.model.urls["delete"], data: jsonParam, success: function (result) {
                    if (result.s) {
...
function GenSubPage(id) {
    var jsonParam = JSON.stringify({ page_id:id});
    parent.ShowLoading("正在生成测序")
    com.ajax({
        url: km.model.urls["gen"], data: jsonParam, success: function (result) {
            parent.HideLoading();
...
/*工具栏权限按钮事件*/
 function edit_sub_page (parent_id, id) {
     ...
     $('#tmp').dialog_ext({
         title: '子页面', iconCls: 'icon-standard-zoom', top: 100, height: 600,width: 1200,
        ...
        onClickButton: function (win ) { //保存操作 
            //    if (editIndexub_page_dialogsearch != undefined)
            //var data = win.find('#sub_page_dialog').datagrid('getData');
            ////alert('总数据量:'+data.total)
            //for (var i = 0; i < data.total; i++) {
            //    if (!win.find('#sub_page_dialog').datagrid('validateRow', i)) {
            //        //  layer.msg('请填查询条件！')
            ...
            //var jsonObject = win.find("#ub_page_dialogcontent").serializeJson();





            //parent.add_ub_page_dialogsearch(false);
            //accept_ub_page_dialogsearch(win);
            ...
            //var options = $('#xx_page_detail_config').datagrid('options')

            //options.url = km.model.urls["ub_page_dialogquery"];
            //options.queryParams = jsonObject;//{ _t: com.settings.timestamp(), crops_id: gcrops_id }; 
            //$('#xx_page_detail_config').datagrid(options);
